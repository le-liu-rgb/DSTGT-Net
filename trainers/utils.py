...
def new_log(logdir, filename):
    ...
    filename = os.path.join(logdir,
                            datetime.datetime.now().strftime(
                                "log_%Y-%m-%d-%H-%M-%S_" + socket.gethostname() + "_" + filename + ".log"))  # 按时间戳和主机名生成日志文件名
...
def haversine(input_coords,
    ...
    a = torch.sin(lat_errors / 2) ** 2  # haversine-a
    +torch.cos(input_coords[:, :, 0]) * torch.cos(pred_coords[:, :, 0]) * torch.sin(
        lon_errors / 2) ** 2
